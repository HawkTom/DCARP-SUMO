...
// the SBX crossover operator
{
    ...
    xed_child->Loads[0] = 0;
    int curr_loads = 0;
    for (i=2; i<=xed_child->Sequence[0]; i++)
    {
        if (xed_child->Sequence[i] == 0)
        {
            xed_child->Loads[0]++;
            xed_child->Loads[xed_child->Loads[0]] = curr_loads;
            curr_loads = 0;
            continue;
        }
        curr_loads += inst_tasks[xed_child->Sequence[i]].demand;
    }
...
void lns1(CARPInd *indi, double coef, int nsize, const Task *inst_tasks)
{
...
void lns_mut(CARPInd *c, CARPInd *p, CARPInd *best_fsb_solution, const Task *inst_tasks)
{
    ...
    check_solution_valid(*c, inst_tasks);
...
void calculate_route_num(int *seq);
void calculate_route_num(int *seq)
{
    int route_num =0;
...
void lns(CARPInd *indi, double coef, int nsize, const Task *inst_tasks)
{
    ...
    if (nsize == 1) // traditional move
    {
        ...
        // if (next_move.type == 2 && next_move.task1==114 &&next_move.task2==110 && next_move.orig_seg == 12 && next_move.targ_seg == 9 && next_move.orig_pos==3 && next_move.targ_pos==9)
        // {
        //     printf("stop\n");
        // }
